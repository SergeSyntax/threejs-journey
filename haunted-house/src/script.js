...
const greaveGeometry = new BoxGeometry(0.6, 0.8, 0.2);
const greaveMaterial = new MeshStandardMaterial({ color: '#b2b6b1' });
...
for (let i = 0; i < 50; i++) {
  ...
  // Get the x position using cosines
  const x = Math.sin(angle) * radius;
  // Get the z position using sinus
  const z = Math.cos(angle) * radius;

  // Create the mesh
  const grave = new Mesh(greaveGeometry, greaveMaterial);
  // Position
